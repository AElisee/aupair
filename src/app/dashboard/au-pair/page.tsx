...
import { User, Search, MessageCircle, Bell, CreditCard, Settings, Home, MapPin, Clock, Eye, Heart } from "lucide-react";
...
const suggestedFamilies = [
  { id: "1", name: "Famille Martin", city: "Lyon", country: "France", flag: "🇫🇷", kids: 2, hoursPerWeek: 30, pocketMoney: 350 },
  { id: "2", name: "Familie Becker", city: "Munich", country: "Allemagne", flag: "🇩🇪", kids: 1, hoursPerWeek: 25, pocketMoney: 380 },
      ...
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-[#1A1A2E]">Complétion du profil</h3>
              <span className="text-[#E87722] font-bold text-lg">{profileCompletion}%</span>
            </div>
            <div className="w-full bg-gray-100 rounded-full h-2 mb-3">
              <div className="bg-[#E87722] h-2 rounded-full transition-all" style={{ width: `${profileCompletion}%` }} />
            </div>
            <p className="text-sm text-gray-500 mb-3">Un profil complet attire 3x plus de familles.</p>
            <Link href="/dashboard/au-pair/profil">
              <Button size="sm" variant="outline">Compléter mon profil</Button>
            </Link>
          </div>
          ...
          <div className={`rounded-2xl p-5 border shadow-sm ${!subscriptionActive ? "bg-red-50 border-red-200" : daysLeft <= 7 ? "bg-yellow-50 border-yellow-200" : "bg-white border-gray-100"}`}>
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-bold text-2xl text-[#1A1A2E]">{daysLeft}</span>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-[#1A1A2E]">Familles suggérées pour vous</h3>
            <Link href="/dashboard/au-pair/recherche" className="text-[#E87722] text-sm font-semibold">Voir toutes →</Link>
          </div>
          <div className="space-y-3">
            {suggestedFamilies.map((f) => (
              <div key={f.id} className="flex items-center justify-between p-3 bg-[#F5F5F5] rounded-xl hover:bg-[#FFF3E0] transition-colors">
                <div className="flex items-center gap-3">
                  <div className="text-2xl">{f.flag}</div>
                  <div>
                    <p className="font-semibold text-[#1A1A2E] text-sm">{f.name}</p>
                    <p className="text-xs text-gray-500 flex items-center gap-1">
                      <MapPin className="w-3 h-3" />{f.city}, {f.country}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-[#E87722]">{f.pocketMoney}€/mois</p>
                  <p className="text-xs text-gray-400">{f.kids} enfant{f.kids > 1 ? "s" : ""} · {f.hoursPerWeek}h/sem</p>
                </div>
              </div>
            ))}
          </div>
